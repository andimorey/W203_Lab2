...
oct_temp <- read.csv("avgtemp_oct2020.csv")

covid_data1<-left_join(covid_raw_data,
                       covid_masks_policies_data)

covid_data <- left_join(covid_data1,
                        oct_temp)
```
...
And here are the variables with NAs.
...
```{r Sixty Five Map, fig.height=3, fig.align='center'}
...
```{r White Map, fig.height=3, fig.align='center'}
...
```{r Risk Map, fig.height=3, fig.align='center'}
...
The coefficient for *white_percent* here might not be answering what we want to observe. The data is at state level, not at patient to be able to see how the outcome of getting infected may differ from one person to another if they below to different races. We picked this variable because of background knowledge we gathered about people of color and people in minorities being hit worse by the pandemic than others.  States with higher white percentage could have had a higher death rate, it could be reflecting that more diverse states with possibly different political leanings took better precautions - wore their masks, took the pandemic more seriously, causing states with lesser white population percentage to have lower death rates. So we will be cautious about how we use this coefficient to understand our question.
...
We lose statistical significance for *mask_mandate_all* indicating instability that *workpl_mob_change* may have added in the model.  This could be because *workpl_mob_change* and *mask_mandate_all* have a common causal parent causing *workpl_mob_change* to share some of the effect and reducing that of *mask_mandate_all* . It could also be that *death_rate_in_last7* has a reverse causal relationship on *workpl_mob_change* (increased death rate would encourage people to decrease workplace mobility). 
We can also see that the significance of the *percent_over_65* variable has increase, still not to a statistically significant level. The correlation between *workpl_mob_change* and *white_percent* also seems to have affected the significance that *white_percent*'s coefficient held in the previous model. This could be due to the political leanings of the states with higher *white_percent* and their view on not taking Covid-19 as seriously. It has introduced another level of instability to the model coefficients.
...
For model 5, we will add two variables on top of what we had in model2 (to avoid the instabilities bought in by the additional variables in model3 and model4). The two variables are:

1. *temp* -  this reflects the average temperature in the state in October 2020. We care about this because we hypothesize that at higher temperatures, people conducted their social activities outdoors which reduces the risk of infection and of higher viral load in case of infection. With the US being a big country the states would see a good deal of variation in temperature which could explain our outcome variable. We chose only October because we are explaining death rate in the last 7 days, which according to the data are last 7 days of October. To have a more direct impact, we chose the monthly average temperature of October.

2. *NE_north* - this is an  indicator variable for the 3 Northern New England states of Vermont, New Hampshire and Maine. We chose this variable because they have a unique characteristic among states with high *white_percent*. Their political leanings (which we thought were to an extent responsible for the effect *white_percent* has on death rate) are different from other states with high *white_percent*. They are progressive as well as libertarian. They seem to have been lower hit by the pandemic in general so we thought it was worth studying what their effect on our outcome variable would be.


```{r Create NE_north data}
covid_data$NE_north <- ifelse(covid_data$State %in% 
                                c("Vermont", "New Hampshire", "Maine"), TRUE, FALSE)
```

$$death\_rate_7 = \beta_0 + \beta_1 mask\_mandate + \beta_2 white\_percent +$$
$$\beta_3  NE\_north + \beta_4 oct\_temp + z \tag{Model 5}$$

```{r Model 5}
model5 <- lm(death_rate_in_last7 ~ mask_mandate_all + white_percent  + NE_north + temperature,
             data = covid_data)
summary(model5)
```

What we observe is that firstly, we have a significant coefficient for *mask_mandate_all* because we lost the variables that introduced instability to the model. We see that the *temp* variable is not statistically significant in explaining our outcome variable and practically has a low effect: every 1 degree decrease in temperature would cause 0.0047 more deaths per 100K or 5 more deaths in 100 million. The *NE_north* variable has a significant explanatory effect though, both statistically and practically. It shows that these states have seen a death rate per 100K lower by 0.4, that means 4 lives per 1 million. 

# Assessing assumptions for our Models
...
```{r IDD Maps, fig.height=8, fig.align='center'}
...
covid_data <- covid_data %>%  mutate(model5_residuals = resid(model5))
covid_data <- covid_data %>%  mutate(model5_predictions = predict(model5))
...
plot_model_5 <- covid_data %>%
  ggplot(aes(x = model5_predictions, y = model5_residuals)) +
  geom_point() + stat_smooth(se=TRUE) +
  labs(
    title = 'Model 5 Residuals
    vs. Model 5 Predictions',
    x = 'Model 5 Predictions',
    y = 'Model 5 Residuals'
  )


plot_model_2 | plot_model_3 
plot_model_4 | plot_model_5
```

By the occular test, we see that model 4 is the most linear between the predictions and residuals and meets this assumption.  We get more linear moving from model 2 to 4, and lost linearity with model 5.
...
This assumption can be affected by issues with assumption 2. If there is a problem with 2, it is likely to show up here as well. Another issue could be skewness of the data. We see in the EDA that there is a bit of skew in the distribution of our outcome variable. Here it seems to not be too bad and we are somewhat safer as we are looking at the death rate over 7 days. Nevertheless, if as we mentioned in 2, there are major discrepancies between states in terms of the exponential growth of the epidemic, there could be orders of magnitude differences in rates and that could be a significant issue. Helpful tools could be transforming the variable (taking a log for example) as well as using robust errors.


```{r Homoskedastic Conditional Errors, dpi=400, fig.height=3.5, fig.width=6,fig.align='center'}

lmtest::bptest(model2)
plot(model2, which=3, labels.id = state.name,
     sub.caption = "Model 2 ACF Plot of the Residuals")

lmtest::bptest(model3)
plot(model3, which=3, labels.id = state.name,
     sub.caption = "Model 3 ACF Plot of the Residuals")

lmtest::bptest(model4)
plot(model4, which=3, labels.id = state.name,
     sub.caption = "Model 4 ACF Plot of the Residuals")

lmtest::bptest(model5)
plot(model5, which=3, labels.id = state.name,
     sub.caption = "Model 5 ACF Plot of the Residuals")
```

As seen with all of our models, the p-value of the Breush-Pagan test makes us not reject the null hypothesis that the variance is constant and we meet assumption 4.
...
```{r Normality of the Errors, fig.height=4, fig.width=7.5, fig.align='center'}
hist(model2$residuals, breaks = 40,
     main = "Residuals from Linear Model 2") # Histogram of the Residuals
plot(model2, which=2, labels.id = state.name,
     sub.caption = "Model 2 QQ Plot of the Residuals")

hist(model3$residuals, breaks = 40,
     main = "Residuals from Linear Model 3") # Histogram of the Residuals
plot(model3, which=2, labels.id = state.name,
     sub.caption = "Model 3 QQ Plot of the Residuals")
...
plot(model4, which=2, labels.id = state.name,
     sub.caption = "Model 4 QQ Plot of the Residuals")

hist(model5$residuals, breaks = 40,
     main = "Residuals from Linear Model 5") # Histogram of the Residuals
plot(model5, which=2, labels.id = state.name,
     sub.caption = "Model 5 QQ Plot of the Residuals")
```

Here we can see that the errors are somewhat normal with one large outlier (Ohio).  It is difficult to get a perfectly normal distribution in such a model with less than 50 degrees of freedom.  This assumption is somewhat met; however, a more robust model could be built at a later time with some transformations or find an indicator variable to make Ohio less of an outlier. 

\newpage

# Regression Table

```{r latex output reg1, warning=FALSE, results='asis'}
stargazer(model1,model2, model3, model4,
          type="latex", column.sep.width = "0pt",
          se = list( sqrt(diag(vcovHC(model1))), sqrt(diag(vcovHC(model2))),
                     sqrt(diag(vcovHC(model3))), sqrt(diag(vcovHC(model4)))) ,
...
We see a reduced residual standard errors from model4 which gives us confidence in a better set of variables chosen to explain death rate in last 7 days. The improved R^2 and adjusted R62 (improved more widely than between model2 and model4) also add our confidence on model5. 
