...
Let us also add an additional indicator variable for the 3 New England states of Vermont, New Hampshire and Maine. We'll call it `NE_north`.
...
$$death\_rate_7 = \beta_0 + \beta_1 mask\_mandate + \beta_2 white\_percent + \beta_3  NE\_north + \beta_4 oct\_temp$$

```{r Model 5}
model5 <- lm(death_rate_in_last7 ~ mask_mandate_all + white_percent  + NE_north + temperature, data = covid_data)
summary(model5)
```
