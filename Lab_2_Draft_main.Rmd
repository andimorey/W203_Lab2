...
plot(model1, which=2) # QQ Plot of Residuals (How do we get QQ plot to show States rather than ID's?)
#plot(model1, which=3) # Heteroskedasticity (looking for a straight line)
#plot(model1, which=5) # Cook's distance
```

When we plot the residuals of our first model, we can see that it isn't quite normal, with one particular state (North Dakota) having a residual above 1, which is quite high given that the median is below 0.5. Then again we don't see clear evidence of violations of assumptions 2 or 4.
...
$$death\_rate_7 = \beta_0 + \beta_1 mask\_mandate + \beta_2 white\_percent + w \tag{2}$$
```{r Model 2}
...
$$death\_rate_7 = \beta_0 + \beta_1 mask\_mandate + \beta_2 white\_percent + \beta_3\ x65 + \beta_4\ percent\_at\_risk+ u \tag{3}$$
...
$$+ \beta_4 workplaces\_mobility\_change + v \tag{3}$$
